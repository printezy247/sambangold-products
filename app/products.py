...
PRODUCTS = (
    ...
    Product(
        ...
        bot_commands=(
            ("/rebateaudit", "summary of your last reconciliation, or a quick LOTS RATE PAID check"),
            ("/rebatestatus", "shortfall total for the current period"),
        ),
        dashboard_views=(
            "CSV paste or upload for the broker statement and the client trade log, rate card with per-symbol rates and tiers",
            "Sortable diff table — shortfall, missing account, silent rate change, excluded symbol",
            "Dispute PDF with the per-account arithmetic shown, and the run history",
        ),
