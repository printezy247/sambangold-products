...
def bot_influencer(args, chat_id=None, lang=DEFAULT_LANG, **_):
    ...
    tail = [t("scan.demand", lang)] + ["• " + d for d in report["demands"]]
...
FORMS = {
    "bot-scam-detector": {"subject_label": "Bot username", "subject_placeholder": "@some_verify_bot",
                          "text_label": "Paste what the bot said (optional)", "needs_subject": True},
    "copy-trade-audit": {"subject_label": "Broker", "subject_placeholder": "Exness, HFM, IC Markets…",
                         "text_label": "Paste the copy-trade pitch (optional)", "needs_subject": True},
    "red-flag-scanner": {"subject_label": "Channel or seller (optional)", "subject_placeholder": "@vip_gold_signals",
                         "text_label": "Paste the pitch", "needs_subject": False},
    "influencer-audit": {"subject_label": "Influencer handle", "subject_placeholder": "@rich_gold_guru",
                         "text_label": "Paste their post, bio or DM (optional)", "needs_subject": True},
}
    ...
    def build(request, user=None):
        ...
        if request.method == "POST" and request.form.get("action") == "loss":
            ...
            if spec["needs_subject"] and not form.get("subject", "").strip():
                ctx["error"] = "%s is required." % spec["subject_label"]
            elif not spec["needs_subject"] and len(form.get("text", "").strip()) < 8:
                ctx["error"] = "Paste the pitch first."
            else:
...
def public_page(scan_id):
    ...
    return {"row": row, "report": row["report"], "flag_icon": FLAG_ICON, "verdict_icon": VERDICT_ICON,
            "verdict_text": verdict_text, "others": store.public_scans(row["product"], row["subject"])}
